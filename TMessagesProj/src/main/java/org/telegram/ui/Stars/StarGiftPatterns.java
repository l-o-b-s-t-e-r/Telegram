...
public class StarGiftPatterns {
    ...
    private static final float[] newProfileRight = new float[]{
            0, -64, 24, 0.32f, 0.7f,
            -42, -82, 20, 0.23f, 0.4f,
            42, -82, 20, 0.23f, 0.4f,
            -108, -50, 20, 0.32f, 0.6f,
            108, -50, 20, 0.32f, 0.6f,
            -64, -32, 24, 0.32f, 0.7f,
            64, -32, 24, 0.32f, 0.7f,
            ...
            -42, 82, 20, 0.23f, 0.0f,
            42, 82, 20, 0.23f, 0.0f,
            -108, 64, 20, 0.32f, 0.6f,
            108, 64, 20, 0.32f, 0.4f,
            -64, 32, 24, 0.32f, 0.7f,
            64, 32, 24, 0.32f, 0.7f,
    };
    ...
    public static void drawNewProfilePattern(Canvas canvas, Drawable pattern, float w, float h, float alpha, float progress) {
        if (alpha <= 0.0f) return;

        final float b = h;
        final float r = w / 2f;

        for (int i = 0; i < newProfileRight.length; i += 5) {
            final float x = newProfileRight[i];
            final float y = newProfileRight[i + 1];
            final float size = newProfileRight[i + 2];
            final float thisAlpha = newProfileRight[i + 3];
            final float animateBefore = newProfileRight[i + 4];

            float newProgress = progress >= animateBefore ? AndroidUtilities.lerp(0f, 1f, (progress - animateBefore) / (1f - animateBefore)) : 0f;
            float newX = AndroidUtilities.lerp(0f, x, newProgress);
            float newY = AndroidUtilities.lerp(0f, y, newProgress);

            pattern.setBounds(
                    (int) (r + dpf2(newX) - dpf2(size) / 2.0f),
                    (int) (b + dpf2(newY) - dpf2(size) / 2.0f),
                    (int) (r + dpf2(newX) + dpf2(size) / 2.0f),
                    (int) (b + dpf2(newY) + dpf2(size) / 2.0f)
            );
